          ...
          <div className="stagger-list mt-8 grid gap-4">
            {[
              ...
              ["Social", contact.instagramUrl || organizer.social]
                ...
                <p className="mt-1 text-lg font-black text-pitch">{value}</p>
              </div>
            ))}
          </div>
          <div className="mt-5 flex flex-wrap gap-3">
            {whatsapp ? (
              <a href={whatsapp} target="_blank" rel="noreferrer" className="shine-button rounded-md bg-[#25D366] px-5 py-3 text-sm font-black text-pitch">
                WhatsApp Organizer
              </a>
            ) : null}
            <a href={contact.mapsUrl} target="_blank" rel="noreferrer" className="shine-button rounded-md bg-pitch px-5 py-3 text-sm font-black text-white">
              Open Venue Map
            </a>
          </div>
        </section>

        <section className="motion-card rounded-lg bg-pitch p-6 text-white shadow-lift">
          <p className="text-sm font-black uppercase tracking-wide text-crease">Hosted Sports</p>
          <h2 className="mt-2 text-3xl font-black">{tournaments.length} tournaments on the hub</h2>
          ...
          <div className="mt-8 grid gap-3">
            <Link href="/tournaments" className="shine-button rounded-md bg-crease px-4 py-3 text-center text-sm font-black text-pitch">
              Browse Tournaments
            </Link>
            <Link href="/register" className="shine-button rounded-md border border-white/18 px-4 py-3 text-center text-sm font-black text-white">
              Register Now
            </Link>
          </div>
        </section>
