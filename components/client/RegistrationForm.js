        ...
        <dl className="mt-5 grid gap-3 text-sm sm:mt-6 sm:gap-4">
          {[
            ...
            <div key={label} className="rounded-md bg-floodlight px-4 py-3">
              ...
              <dd className="mt-1 break-words font-black text-pitch">{value}</dd>
            </div>
          ))}
        </dl>
      </aside>

      <form onSubmit={submit} className="motion-card rounded-lg border border-graphite/10 bg-white p-4 shadow-sm sm:p-6">
        ...
        <label className="mt-4 flex gap-3 rounded-lg bg-floodlight p-4 text-sm font-bold text-pitch">
          <input name="termsAccepted" type="checkbox" checked={form.termsAccepted} onChange={updateField} required className="mt-1 h-5 min-h-0 w-5 shrink-0" />
          <span>
            I confirm the details are correct and accept the tournament rules, privacy policy, and refund/cancellation policy.
          </span>
        </label>
        <button disabled={busy} className="shine-button tap-target mt-6 w-full rounded-md bg-pitch px-5 py-3 text-sm font-black text-white transition hover:bg-scoreboard disabled:cursor-not-allowed disabled:opacity-60">
