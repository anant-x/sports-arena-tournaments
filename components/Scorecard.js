...
export default function Scorecard({ match, tournament }) {
  ...
  return (
    <article className="motion-card rounded-lg border border-graphite/10 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-2 border-b border-graphite/10 pb-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm font-black uppercase text-turf">{match.date} · {match.time}</p>
          <h3 className="mt-1 text-xl font-black text-pitch">{displayName(match.team1, tournament)} vs {displayName(match.team2, tournament)}</h3>
          <p className="mt-1 text-sm text-graphite/62">{match.venue}</p>
        </div>
        <span className="rounded-md bg-scoreboard px-3 py-1.5 text-sm font-black text-white">{match.result}</span>
      </div>
