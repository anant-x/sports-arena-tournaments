    ...
    for (const match of tournament.matches || []) {
      await sql`
        ...
        ON CONFLICT (tournament_slug, match_id) DO UPDATE SET
          team1 = CASE
            WHEN match_updates.team1 = ''
              OR match_updates.team1 = 'TBD'
              OR match_updates.team1 ~* '^Team[[:space:]][0-9]+$'
              OR match_updates.team1 ~* '^Finalist[[:space:]]'
              OR match_updates.team1 ~* '^Winner Match[[:space:]]'
              OR match_updates.team1 ~* '^Court[[:space:]]'
              OR match_updates.team1 ~* '^Semi[[:space:]]'
              OR match_updates.team1 ~* '^Doubles[[:space:]][A-Z]$'
              OR match_updates.team1 ~* '^Corp[[:space:]][A-Z]$'
            THEN EXCLUDED.team1
            ELSE match_updates.team1
          END,
          team2 = CASE
            WHEN match_updates.team2 = ''
              OR match_updates.team2 = 'TBD'
              OR match_updates.team2 ~* '^Team[[:space:]][0-9]+$'
              OR match_updates.team2 ~* '^Finalist[[:space:]]'
              OR match_updates.team2 ~* '^Winner Match[[:space:]]'
              OR match_updates.team2 ~* '^Court[[:space:]]'
              OR match_updates.team2 ~* '^Semi[[:space:]]'
              OR match_updates.team2 ~* '^Doubles[[:space:]][A-Z]$'
              OR match_updates.team2 ~* '^Corp[[:space:]][A-Z]$'
            THEN EXCLUDED.team2
            ELSE match_updates.team2
          END,
          date = EXCLUDED.date,
          time = EXCLUDED.time,
          venue = EXCLUDED.venue,
          status = CASE
            WHEN match_updates.status = 'upcoming' THEN EXCLUDED.status
            ELSE match_updates.status
          END,
          result = CASE
            WHEN match_updates.result = '' THEN EXCLUDED.result
            ELSE match_updates.result
          END,
          scorecard = CASE
            WHEN match_updates.scorecard = '{}'::jsonb THEN EXCLUDED.scorecard
            ELSE match_updates.scorecard
          END,
          updated_at = NOW()
      `;
    }
